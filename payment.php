 <?php
  ...
  if(!empty($_GET)){
    if($_GET['act'] == 'delete'){
      ...
      $q = mysqli_query($connect, "delete from pesanan WHERE id='$_GET[id]'");
      if($q){ alert("Success"); redir("payment.php"); }
    }
  }
...
<?php
		if($_GET['act'] == 'bayar' && $_GET['id']){
			...
			extract($_GET);
			$pesanan = mysqli_fetch_object(mysqli_query($connect, "Select*from pesanan where id='$id'"));
			$qPembayaran = mysqli_query($connect, "Select * from pembayaran where id_pesanan='$id' and status='verified'") or die (mysqli_error($d));
			$totalPembayaran = 0;
