<?php 
	...
	if(!empty($_GET['act']) && $_GET['act'] == 'edit'){
		if(!empty($_POST)){
			...
			$q = mysqli_query($connect,"update pesanan SET nama='$nama',tanggal_pesan='$tanggal_pesan',tanggal_digunakan='$tanggal_digunakan',user_id='$user_id',alamat='$alamat',telephone='$telephone' where id=$_GET[id]") or die(mysqli_error($q));
			if($q){ alert("Success"); redir("pesanan.php"); }
		}
	}
...
?> 
	...
	<div class="container">
		...
		<?php
			if(!empty($_GET)){ 
				if($_GET['act'] == 'edit'){
					$data = mysqli_fetch_object(mysqli_query($connect,"select*from pesanan where id='$_GET[id]'"));
				?>
