...
?> 
	...
	<div class="container">
		...
		<?php
			if(!empty($_GET)){
				if ($_GET['act'] == "info") {
					$qInfo = mysqli_query($connect,"Select * from info_pembayaran limit 1") or die (mysql_error());
					$dInfo = mysqli_fetch_object($qInfo);
					if (!empty($_POST)) {
						extract($_POST);
						$query = mysqli_query($connect,"Update info_pembayaran set info='$info' where id='1'") or die (mysqli_error($dInfo));
						if ($query) {
				...
				if ($_GET['act'] == "verified") {
					extract($_GET);
					$updatePembayaran = mysqli_query($connect,"update pembayaran set status = 'verified' where id='$_GET[id]'");
					if ($updatePembayaran) {
						$qPembayaran = mysqli_query($connect,"Select * from pembayaran where id_pesanan='$id_pesanan' and status='verified'") or die (mysql_error());
						$totalPembayaran = 0;
						while ($d = mysqli_fetch_object($qPembayaran)) {
							$totalPembayaran += $d->total;
						}
						$q1 = mysqli_query($connect,"select*from detail_pesanan where pesanan_id='$id_pesanan'");
						$totalBayar = 0;
						...
						$q3 = mysqli_query($connect,"select*from pesanan where id='$id_pesanan'") or die (mysqli_error($q3));
						$pesanan = mysqli_fetch_object($q3);

						$totalBayar += $pesanan->ongkir;
						if ($totalBayar <= $totalPembayaran) {
							$updatePesanan = mysqli_query($connect,"update pesanan set status='lunas' where id='$id_pesanan'") or die (mysqli_error($pesanan));
							if ($updatePesanan) {
				...
				if ($_GET['act'] == "detail") {
					$dataPembayaran = mysqli_fetch_object(mysqli_query($connect,"Select * from pembayaran where id=$_GET[id]"));
		...
		<?php while($data=mysqli_fetch_object($q)){ ?> 
				<tr <?php if($data->status == "pending" ){ echo 'style="background:#cce9f8 !important;"'; } ?> > 
					...
					<?php
						$katpro = mysqli_query($connect,"select*from user where id='$data->id_user'");
