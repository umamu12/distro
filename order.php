...
?>
		...
		<?php
			if(!empty($_POST['form-order'])){
				...
				$kt = mysqli_fetch_object(mysqli_query($connect, "SELECT *FROM kota where id='$kota'"));
				if($difference->days >= 3){
					
					$q = mysqli_query($connect, "insert into pesanan Values(NULL,'$tanggal_pesan','$tanggal_digunakan','$_SESSION[iam_user]','$nama','$alamat','$kt->nama','$kt->ongkir','$telephone','0','belum lunas')");
					if($q){

						$last = mysqli_fetch_array(mysqli_query($connect, "SELECT *FROM pesanan order by id DESC limit 1"));
						...
						foreach($cart as $id => $qty){
							...
							if(!empty($product)){
								...
								$ins = mysqli_query($connect, "insert into detail_pesanan Values(NULL,'$product[id]','$qty','$last[id]')");
					...
					?>
					<div class="alert alert-danger">Usage date is too tight. Order at least 3 days before the day.</div>
					<?php
				}
			}
		?>
			...
			<div class="col-md-7">
				...
				<hr>
				 <form action="" method="post" enctype="multipart/form-data">
						...
						><br>
						<label>Telephone (phone)</label><br>
